...
namespace Zxurian\AdobeSign;

use GuzzleHttp\Psr7\MultipartStream;
use Zxurian\AdobeSign\Exceptions\AdobeSignException;
use Zxurian\AdobeSign\Exceptions\AdobeSignInvalidAccessTokenException;
use Zxurian\AdobeSign\Exceptions\AdobeSignMissingRequiredParamException;
use Zxurian\AdobeSign\Exceptions\AdobeSignUnsupportedMediaTypeException;
use League\OAuth2\Client\Provider\AbstractProvider;
use League\OAuth2\Client\Tool\QueryBuilderTrait;
use Zxurian\AdobeSign\Operations\BaseUris;

/**
 * Class AdobeSign
 * @package Zxurian\AdobeSign
 */
class AdobeSign
{
    ...
    /**
     * @var \Zxurian\OAuth2\Client\AdobeSign
     */
    protected $provider;
    ...
    /**
     * Parse the response and return a the specified response type with filled information
     * 
     * @param string $operation The class that is performing the operation
     * @param string $responseType The class specifying the response type
     * @param array $response The response from the provider in an associative array
     * @throws AdobeSignException
     * @return BaseUris
     */
    protected function parseResponse($operation, $responseType, $response)
    {
        if (key_exists('code', $res)) {
            $exceptionClass = $operation.'Exception';
            throw new $exceptionClass($exceptionClass::getReason($code), $exceptionClass($code));
        }
